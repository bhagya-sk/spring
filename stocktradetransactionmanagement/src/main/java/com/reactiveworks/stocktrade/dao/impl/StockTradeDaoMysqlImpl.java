...
public class StockTradeDaoMysqlImpl extends JdbcDaoSupport implements IStockTradeDao {
	 ...
	 * @throws DaoOperationFailureException
	...
	public List<StockTrade> getStockTradeRecords() throws StockTradeDaoException {
		...
		try {
			...
			stockTrdList = transactionTemplate.execute(new TransactionCallback<List<StockTrade>>() {

				@Override
				public List<StockTrade> doInTransaction(TransactionStatus status) {
					List<StockTrade> stockTrdList = getJdbcTemplate().query(SELECT_QUERY, new StockTradeMapper());
					return stockTrdList;
			...
			LOGGER_OBJ.debug("transaction failed");
			throw new StockTradeDaoException("unable to get the stocktrade record from the database", transactionExp);
		}
	...
	public void createStockTradeRecord(StockTrade stockTradeObj) throws StockTradeDaoException {
		...
		LOGGER_OBJ.info("stockTrade is record is inserted into the database.");
					...
					getJdbcTemplate().batchUpdate(INSERT_QUERY, new BatchPreparedStatementSetter() {
						...
						public void setValues(PreparedStatement preparedStatement, int index) {
							...
							try {
								...
								LOGGER_OBJ.error("unable to insert stockTrade object with id " + (index + 1)
										+ " into the database");
							}
	...
	public void deleteStockTradeRecord(int stockTradeId) throws StockTradeDaoException {
        ...
        LOGGER_OBJ.info("stockTrade record with id "+stockTradeId+" is deleted.");
	...
	public void updateStockTradeRecord(int stockTrdId, double volume) throws StockTradeDaoException {
		...
		LOGGER_OBJ.info("stocktrade record with id "+stockTrdId+" is updated");
